...
function addSale() {
    ...
    if (add_saleName == null || add_saleName == "" || add_salePercentage == null || add_salePercentage == "") {
        ...
        var num = parseFloat(add_salePercentage);
        if (Number.isInteger(num)) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if(data.saleExist===true){
                        alert("Sale cho sản phẩm này đã tồn tại");
                    }
...
function saleToExcel() {
    ...
    for (let i = 0; i < table.rows.length; i++) {
        ...
        let onSale = currentRow.cells[3].textContent;
        let discount = currentRow.cells[4].textContent;
...
function updateSale() {
    ...
    if (update_saleName == null || update_saleName == "" || update_salePercentage == null || update_salePercentage == "") {
        ...
        var num = parseFloat(update_salePercentage);
        if(Number.isInteger(num)){
            $.ajax({
                type: "POST",
                url: adminManagerContextPath + "/ManageSaleServlet",
...
function searchAndSortSale() {
    ...
    if (saleSearchType == "id") {
        var num = parseFloat(saleInputSearch);
        if (Number.isInteger(num)) {
            return true;
        } else {
            alert("Dữ liệu điền vào không hợp lệ");
